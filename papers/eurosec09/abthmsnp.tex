...
The dotted arrows represent the packets the receiving hosts sends as a response to the injected packets.
These packets are still received by the other end of the connection but are largely ignored as, in the perspective of the receiver, the packet contains an incorrect seqnum and acknum.
...
Gathered from empircal tests, we summarize three TCP stacks in Table~\ref{tab:implementations}: an ideal (non-existent) RFC specified interpretation of TCP packet processing (as shown in~\cite{zaghal:model}), and the TCP stack implementation on Microsoft Windows and Linux.
